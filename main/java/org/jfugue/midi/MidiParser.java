...
public class MidiParser extends Parser
{
    ...
    public void parseEvent(MidiEvent event) {
        ...
        if ((status >= 128) && (status <= 239)) {
            int messageId = (status-128) / MidiDefaults.TRACKS; 
            int channel = status % MidiDefaults.TRACKS;
    ...
    private void noteOff(int channel, MidiEvent event) {
        ...
        System.out.println("Duration="+noteObject.getDuration()+"   event.getTick()="+event.getTick()+"    tempNote.startTick="+tempNote.startTick); // TODO Diagnostic line, please remove
