 ...
 * 
 * THIS IS A NEW IMPLEMENTATION OF MidiEventManager THAT DELEGATES TO THE NEW TrackTimeManager
 * AS YET UNTESTED
 * 
 *@author David Koelle
 */
final class MidiEventManager2 extends TrackTimeManager
{
    ...
    public MidiEventManager2() { 
    	setDefaults();
    }
    
    public MidiEventManager2(float divisionType, int resolution) {
    ...
    public void addNote(Note note) {
    	...
    	if (!note.isEndOfTie()) {
    		addEvent(ShortMessage.NOTE_ON, note.getValue(), note.getAttackVelocity());
    	}
    	...
    	if (!note.isStartOfTie()) {
    		addEvent(ShortMessage.NOTE_OFF, note.getValue(), note.getDecayVelocity());
    	}
    }
