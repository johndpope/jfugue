   ...
   /** Parses MIDI file to calculate statistics for all descriptors
    ...
    private boolean checkHarmonics(Note note){ 
        ...
        if (key.getScale().getMajorOrMinorIndicator()==0){ //0 for major
            keyScale = majors.get(reduceValue(key.getRootNote().getValue()));         
        }
        else{
            keyScale = minors.get(reduceValue(key.getRootNote().getValue())); 
        }
    ...
    private void checkDegree(Number n){
        Integer rootValue = reduceValue(key.getRootNote().getValue());
        ...
        for (GetPatternStats.TimeEvent t : musicEvents){
            ...
            if (t.getEvent() instanceof Note || t.getEvent() instanceof org.jfugue.theory.Chord){
                ...
                if (!note.isRest()){
                    ...
                    attacks.add(note.getAttackVelocity());
                    decays.add(note.getDecayVelocity());
