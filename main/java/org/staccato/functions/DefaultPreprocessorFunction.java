	...
	public String apply(String parameters, StaccatoParserContext context) {
		...
		for (String defaultSetting : defaultSettings) {
			...
			if (key.equalsIgnoreCase(OCTAVE)) {
				DefaultManager.getInstance().setDefaultOctave(Byte.parseByte(value));
			} else if (key.equalsIgnoreCase(BASE_OCTAVE)) {
				DefaultManager.getInstance().setDefaultBassOctave(Byte.parseByte(value));
			} else if (key.equalsIgnoreCase(DURATION)) {
				double dur = 0.0d;
				try {
					dur = Double.parseDouble(value);
					DefaultManager.getInstance().setDefaultDuration(dur);
				...
				DefaultManager.getInstance().setDefaultAttackVelocity(Byte.parseByte(value));
			} else if (key.equalsIgnoreCase(DECAY)) {
				DefaultManager.getInstance().setDefaultDecayVelocity(Byte.parseByte(value));
			} else {
