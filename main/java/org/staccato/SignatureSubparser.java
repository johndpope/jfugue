...
public class SignatureSubparser implements Subparser, KeyProvider 
{
	...
	public int parse(String music, StaccatoParserContext context) {
		if (matchesKeySignature(music)) {
			...
			context.setKey(key);
			context.getParser().fireKeySignatureParsed(key.getRootNote().getPositionInOctave(), key.getScale().getMajorOrMinorIndicator());
	...
	public Key createKey(String keySignature) {
	    if (keySignature.indexOf(SHARP_CHAR) > 0 || (keySignature.toUpperCase().indexOf(FLAT_CHAR) > 0)) {
	...
	// TODO createKeyFromAccidentals to allow like "K####"
	public Key createKeyFromAccidentals(String keySignature) {
	    if (keySignature.indexOf(SHARP_CHAR) > 0) {
	        return new Key(createKeyString((byte)keySignature.length(), Scale.MAJOR_INDICATOR));
	    } else {
	        return new Key(createKeyString((byte)keySignature.length(), Scale.MINOR_INDICATOR));
	    }
	}
	
	@Override
	public String createKeyString(byte key, byte scale) {
        StringBuilder buddy = new StringBuilder();
        if (scale == 0) {
            buddy.append(majorSigs[key+7]);
            buddy.append(MAJOR_ABBR);
        } else {
            buddy.append(minorSigs[key+7]);
	...
	public byte convertKeyToByte(Key key) {
		String noteName = key.getRootNote().getOriginalString();
		if (noteName == null) {
			return 0;
		}
		for (byte b = -7; b < +8; b++) {
			if (noteName.equalsIgnoreCase((key.getScale() == Scale.MAJOR) ? majorSigs[7 + b] : minorSigs[7 + b])) {
				return b;
			}
		}
		return 0;
    ...
    public static final String[] majorSigs = new String[] { "Cb", "Gb", "Db", "Ab", "Eb", "Bb", "F", "C", "G", "D", "A", "E", "B", "F#", "C#" };
    public static final String[] minorSigs = new String[] { "Ab", "Eb", "Bb", "F", "C", "G", "D", "A", "E", "B", "F#", "C#", "G#", "D#", "A#" };
