...
//            }
    ...
    private void setDefaultOctave(NoteContext context) {
        ...
        if (context.isChord) {
            context.octaveNumber = DefaultManager.getInstance().getDefaultBassOctave() + context.octaveBias;
        } else {
            context.octaveNumber = DefaultManager.getInstance().getDefaultOctave() + context.octaveBias;
        }
    }
    ...
    private int parseDuration(String s, int index, NoteContext noteContext, StaccatoParserContext parserContext) {
        if (index < s.length()) {
            switch (s.charAt(index)) {
                ...
                default : noteContext.decimalDuration = DefaultManager.getInstance().getDefaultDuration(); noteContext.durationExplicitlySet = false; break; // Could get here if the next character is a velocity char ("a" or "d")
            }
            index = parseTuplet(s, index, noteContext); 
        } else {
        	noteContext.decimalDuration = DefaultManager.getInstance().getDefaultDuration();
...
//                    case '*' : durationNumber = parserContext.getTimeSignature().getDurationForBeat(); // TODO - Default duration based on beat
    ...
    private int parseVelocity(String s, int index, NoteContext context) {
        ...
        while (index < s.length()) {
            ...
            switch (velocityChar) {
                case 'A' : if (velocityString == null) { context.attackVelocity = velocityNumber; } else { context.attackVelocityValueAsString = velocityString; } context.hasAttackVelocity = true; break;
                case 'D' : if (velocityString == null) { context.decayVelocity = velocityNumber; } else { context.decayVelocityValueAsString = velocityString; } context.hasDecayVelocity = true; break;
                default  : throw new ParserException(StaccatoMessages.VELOCITY_CHARACTER_NOT_RECOGNIZED, s.substring(startPoint,endPoint));
            }
            index = endPoint;
        }
        
        if (context.hasAttackVelocity) { logger.info("Attack velocity = " + context.attackVelocity); }
        if (context.hasDecayVelocity) { logger.info("Decay velocity = " + context.decayVelocity); }
    ...
    class NoteContext {
    	...
    	public boolean hasAttackVelocity;
    	public byte attackVelocity;
    	public String attackVelocityValueAsString;
    	public boolean hasDecayVelocity;
        public byte decayVelocity;
        public String decayVelocityValueAsString;
        ...
        public Note createNote(StaccatoParserContext parserContext) {
            ...
            if (hasAttackVelocity) {
	            if (attackVelocityValueAsString != null) {
	            	attackVelocity = (Byte)parserContext.getDictionary().get(attackVelocityValueAsString);
	            }
	            note.setAttackVelocity(attackVelocity);
            }
            
            if (hasDecayVelocity) {
	            if (decayVelocityValueAsString != null) {
	            	decayVelocity = (Byte)parserContext.getDictionary().get(decayVelocityValueAsString);
	            }
	            note.setDecayVelocity(decayVelocity);
            }
