...
public class NoteSubparserTest extends JFugueTestHelper 
{
    ...
    public void testSimpleNotes() {
        ...
        assertTrue(compare("AWHA90", NOTE_PARSED, new Note(69, 1.5d).setAttackVelocity((byte)90)));
    }
    ...
    public void testDyanmicsUsingLetters() {
        assertTrue(compare("Ca20", NOTE_PARSED, new Note(60).setAttackVelocity((byte)20)));
        assertTrue(compare("D4d20", NOTE_PARSED, new Note(50).setDecayVelocity((byte)20)));
        assertTrue(compare("Bb3a10d20", NOTE_PARSED, new Note(46).setAttackVelocity((byte)10).setDecayVelocity((byte)20)));
    }

    @Test
    public void testDyanmicsWithDurations() {
        assertTrue(compare("Ch.a20", NOTE_PARSED, new Note(60).setDuration(0.75d).setAttackVelocity((byte)20)));
        assertTrue(compare("c2w2a2", NOTE_PARSED, new Note(24).setDuration(2.0d).setAttackVelocity((byte)2)));
        assertTrue(compare("D4/0.25d20", NOTE_PARSED, new Note(50).setDuration(0.25d).setDecayVelocity((byte)20)));
        assertTrue(compare("Bb3/-1.0-a10d20", NOTE_PARSED, new Note(46).setDuration(1.0d).setStartOfTie(true).setEndOfTie(true).setAttackVelocity((byte)10).setDecayVelocity((byte)20)));
    }

    // TODO Need to make chord tests work in NoteSubparserTest
    @Ignore @Test
    public void testSimpleChord1() {
        assertTrue(compare("C4maj", CHORD_PARSED, new Chord(new Note(60), Chord.getIntervals("MAJ"))));
    }
    
    // TODO Need to make chord tests work in NoteSubparserTest
    @Ignore @Test
    public void testSimpleChord2() {
        assertTrue(compare("AMIN/1.5", CHORD_PARSED, new Chord(new Note(69).setDuration(1.5d), Chord.getIntervals("MIN"))));
    }
    ...
    public void testChordInversions() {
    	// compare()'s CHORD_PARSED isn't working correctly (seems that when a chord is parsed, notes are fired instead of a chord)
    	assertTrue(compare("C4maj", CHORD_PARSED, new Chord(new Note(60), Chord.getIntervals("MAJ"))));
    }
    ...
    @Ignore @Test
    public void testBeatDuration() {
        assertTrue(compare("C*", NOTE_PARSED, new Note(60, 0.25d)));
    }
