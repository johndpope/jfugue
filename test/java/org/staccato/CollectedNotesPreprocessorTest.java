...
public class CollectedNotesPreprocessorTest {
    ...
    public void testCollectedNotesWithPlusAndSpace() {
        assertTrue(pe.preprocess("Ki (C+E G)q Fw", context).equalsIgnoreCase("Ki Cq+Eq Gq Fw"));
    }
        
    @Test
    public void testMultipleCollectedNotesInSameString() {
        assertTrue(pe.preprocess("Ki (1+2 3)q Fw (4+5 6)q Ko", context).equalsIgnoreCase("Ki 1q+2q 3q Fw 4q+5q 6q Ko"));
    }
        
    @Test
    public void testMultipleCollectedNotesInSameStringWithDecimalDuration() {
        assertTrue(pe.preprocess("Ki (1+2 3)/4.0 Fw (4+5 6)/0.5 Ko", context).equalsIgnoreCase("Ki 1/4.0+2/4.0 3/4.0 Fw 4/0.5+5/0.5 6/0.5 Ko"));
    }

    @Test
    public void testNothingAfterParens() {
        assertTrue(pe.preprocess("Ki (1+2 3) Fw (4+5 6) Ko", context).equalsIgnoreCase("Ki (1+2 3) Fw (4+5 6) Ko"));
    }
}
